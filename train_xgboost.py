...
Trains an XGBoost classifier on the API schema extraction dataset.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Train XGBoost model for schema extraction prediction"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Early stopping rounds (requires eval set)'
    )
    ...
    if args.tune:
        ...
        eval_set = [(X_test, y_test)] if args.early_stopping else None
        model.fit(
            X_train, y_train,
    ...
    # Training metrics
    train_metrics = evaluator.compute_metrics(y_train, y_pred_train, "XGBoost_Train")
    
    # Test metrics
    test_metrics = evaluator.compute_metrics(y_test, y_pred_test, "XGBoost_Test")
    ...
    # Save model
    model_path = args.output_dir / 'xgboost_model.pkl'
    model.save(model_path)
    
    # Save confusion matrix plot
    cm_path = args.output_dir / 'xgboost_confusion_matrix.png'
    ...
    results = {
        ...
        'hyperparameters': {
            ...
            'subsample': model.model.subsample,
            'colsample_bytree': model.model.colsample_bytree,
        }
    }
    ...
    evaluator.save_results(args.output_dir / 'xgboost_results.json', {'XGBoost': results})
